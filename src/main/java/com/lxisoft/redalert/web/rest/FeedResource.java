...
public class FeedResource {
	 ...
	 *
	 * @param feedDTO
	 *            the feedDTO to update
	 * @return the ResponseEntity with status 200 (OK) and with body the updated
	 *         feedDTO, or with status 400 (Bad Request) if the feedDTO is not
	 *         valid, or with status 500 (Internal Server Error) if the feedDTO
	 *         couldn't be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	...
	public ResponseEntity<FeedDTO> updateFeed(@RequestBody FeedDTO feedDTO) throws URISyntaxException {
		...
		FeedDTO result = feedService.save(feedDTO);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, feedDTO.getId().toString()))
				.body(result);
	}
	 ...
	 *
	 * @param id
	 *            the id of the feedDTO to retrieve
	 * @return the ResponseEntity with status 200 (OK) and with body the
	 *         feedDTO, or with status 404 (Not Found)
	 */
	@GetMapping("/feeds/{id}")
	@Timed
	public ResponseEntity<FeedDTO> getFeed(@PathVariable Long id) {
		log.debug("REST request to get Feed : {}", id);
		FeedDTO feedDTO = feedService.findOne(id);
		return ResponseUtil.wrapOrNotFound(Optional.ofNullable(feedDTO));
	}
	 ...
	 *
	 * @param id
	 *            the id of the feedDTO to delete
